...
// 📌 Component แจ้งเตือนให้หมุนจอเป็นแนวนอนบนมือถือ
function OrientationWarning() {
  return (
    <div className="fixed inset-0 z-[999] flex flex-col items-center justify-center bg-zinc-900 text-white p-8 text-center md:hidden landscape:hidden animate-fade-in">
      <div className="w-20 h-20 mb-6 animate-bounce">
        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" className="w-full h-full text-emerald-500">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" />
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13 3h-2a2 2 0 00-2 2v14a2 2 0 002 2h2a2 2 0 002-2V5a2 2 0 00-2-2z" className="opacity-40" />
        </svg>
      </div>
      <h3 className="text-2xl font-black mb-3 italic">กรุณาใช้งานในแนวนอน</h3>
      <p className="text-zinc-400 font-medium leading-relaxed">
        เพื่อการแสดงผลแผนที่และข้อมูลที่สมบูรณ์ที่สุด<br/>
        โปรดหมุนโทรศัพท์ของคุณเป็นแนวนอน
      </p>
    </div>
  );
}
...
export default function ThailandMap() {
  ...
  return (
    <>
      {/* 📌 แจ้งเตือนบังคับแนวนอนบนมือถือ */}
      <OrientationWarning />

      {/* 📌 เนื้อหาหลัก จะถูกซ่อนในแนวตั้ง และแสดงในแนวนอนหรือบนจอคอม */}
      <div className="hidden landscape:flex md:flex flex-col md:flex-row h-[100dvh] w-full overflow-hidden bg-zinc-50 text-zinc-900 relative">
        ...
        {/* แผนที่ซ้ายมือ */}
        <div className="relative w-full h-[45vh] md:h-full md:flex-1 flex items-center justify-center p-2 md:p-4 bg-white shadow-inner overflow-hidden shrink-0">
          
          <div className="absolute top-2 left-2 md:top-6 md:left-6 bg-white/90 backdrop-blur-md p-3 md:p-5 rounded-2xl shadow-sm border border-zinc-200 z-10 pointer-events-none scale-75 md:scale-100 origin-top-left">
          ...
          <div className="absolute bottom-4 right-4 md:bottom-8 md:right-8 flex gap-1 bg-white p-1 rounded-xl shadow-md border border-zinc-200">
        ...
        <div className="w-full flex-1 md:h-full md:w-[380px] lg:w-[450px] border-t md:border-t-0 md:border-l border-zinc-200 p-6 md:p-10 flex flex-col bg-white z-20 shadow-[0_-15px_30px_rgba(0,0,0,0.06)] md:shadow-[-10px_0_30px_rgba(0,0,0,0.02)] overflow-hidden">
          ...
          <div className="relative z-20 mb-4 md:mb-6 shrink-0">
            ...
            {showSearchDropdown && searchTerm && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-zinc-100 rounded-xl shadow-xl max-h-40 md:max-h-60 overflow-y-auto overflow-hidden">
                {filteredProvinces.length > 0 ? (
                  filteredProvinces.map(prov => (
                    <button key={prov} onClick={() => { setSelectedProvince(prov); setSearchTerm(""); setShowSearchDropdown(false); }} className="w-full text-left px-4 py-3 hover:bg-emerald-50 focus:bg-emerald-50 transition-colors text-sm md:text-base text-zinc-800 font-medium cursor-pointer border-b border-zinc-50 last:border-0">
                      {prov}
                    </button>
                  ))
          ...
          <div className="flex-1 overflow-y-auto pr-2 pb-6 md:pb-10">
            {selectedProvince && provinceData ? (
              <article className="space-y-4 md:space-y-6 animate-fade-in">
                ...
                <div className="flex flex-col md:flex-row gap-2 md:gap-3 mt-4">
                  ...
                  <button onClick={() => setSelectedProvince(null)} className="px-6 py-2.5 md:py-3 rounded-xl bg-zinc-100 text-zinc-700 font-semibold text-sm hover:bg-zinc-200 transition cursor-pointer">ปิด</button>
                </div>
              </article>
            ) : (
              <div className="h-full flex flex-col items-center justify-start pt-2 pb-10 animate-fade-in">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-full bg-emerald-50 flex items-center justify-center mb-4 md:mb-6 border border-emerald-100 shadow-sm">
                ...
                <div className="w-full border-t border-zinc-100 pt-6 md:pt-8">
                  ...
                  <div className="grid grid-cols-2 gap-2 md:gap-3">
                    {Object.keys(regionsConfig).map(region => (
                      <button key={region} onClick={() => setSelectedRegion(region)} className="px-3 md:px-4 py-2.5 md:py-3 text-left rounded-xl border border-zinc-200 bg-white hover:border-emerald-400 hover:bg-emerald-50 hover:text-emerald-700 text-zinc-700 font-bold text-xs md:text-sm transition-all shadow-sm cursor-pointer">
                        {region}
                      </button>
                    ))}
                  </div>
